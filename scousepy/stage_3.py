...
# Ensure to_vel_freq is defined at the top level
def to_vel_freq():
...
def process_saa(inputlist):

    from .model_housing import individual_spectrum
    scouseobject, i, j, SAA = inputlist
...
def initialise_fitting(scouseobject):
    ...
    for i in range(len(scouseobject.wsaa)):
        ...
        inputlist = [(scouseobject, i, j, SAA) for j, SAA in saa_dict.items()]

        # if njobs > 1 run in parallel else in series
        if scouseobject.njobs > 1:
            partial_results = parallel_map(process_saa, inputlist, numcores=scouseobject.njobs, verbose=scouseobject.verbose)
        else:
            if scouseobject.verbose:
                partial_results=[process_saa(input) for input in tqdm(inputlist)]
            else:
    ...
    if not is_flat_list(indivspec_list):
        indivspec_list = list(chain.from_iterable(indivspec_list))

    template = gen_template(scouseobject)
    for indivspec in indivspec_list:
